...
class DownloadTask:
    def __init__(self, fileStorage: FileStorage, send_func, selfPort=None, client=None):
        ...
        # multiprocessing.Process(target=self._autoAsk).start()
        multiprocessing.Process(target=self.go).start()

    def go(self):
        threading.Thread(target=self._autoAsk).start()
        multiprocessing.Process(target=self.run).run()
