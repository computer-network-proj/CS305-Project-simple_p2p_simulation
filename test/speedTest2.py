...
def client_download(client):
    client.download("../test_files/binfile.bin")


if __name__ == '__main__':
    # A, B, C, D, E join the network
    A = PClient(tracker_address, upload_rate=100000, download_rate=100000)
    B = PClient(tracker_address, upload_rate=100000, download_rate=40000)
    C = PClient(tracker_address, upload_rate=100000, download_rate=80000)
    D = PClient(tracker_address, upload_rate=100000, download_rate=80000)
    E = PClient(tracker_address, upload_rate=100000, download_rate=200000)
    fid = A.register("../test_files/binfile.bin")
    files = {}
    clients = [B, C, D, E]
    threads = []
    ...
    with open("../test_files/binfile.bin", "rb") as bg:
        bs = bg.read()
        for i in files:
            if files[i] != bs:
                raise Exception()
